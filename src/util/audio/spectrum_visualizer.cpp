...
void spectrum_visualizer::tick(float seconds)
{
    ...
    bool is_silent_left = true,
         is_silent_right = true;

    if (m_cfg->channel == CM_BOTH) {
        is_silent_left = prepare_fft_input(m_cfg->buffer, m_cfg->sample_size,
            m_fftw_input_left, CM_LEFT);
        is_silent_right = prepare_fft_input(m_cfg->buffer, m_cfg->sample_rate,
            m_fftw_input_right, CM_RIGHT);
    } else {
        is_silent_left = prepare_fft_input(m_cfg->buffer, m_cfg->sample_size,
            m_fftw_input_left, CM_LEFT);
    }
    ...
    if (m_silent_runs < 30) {
        ...
        m_fftw_plan_left = fftw_plan_dft_r2c_1d(
            static_cast<int>(m_cfg->sample_size), m_fftw_input_left,
            m_fftw_output_left, FFTW_ESTIMATE);
        if (!m_fftw_plan_left)
            return;
        if (m_cfg->channel == CM_BOTH) {
            m_fftw_plan_right = fftw_plan_dft_r2c_1d(
                static_cast<int>(m_cfg->sample_size), m_fftw_input_right,
...
void spectrum_visualizer::apply_falloff(const doublev& bars,
...
void spectrum_visualizer::maybe_reset_scaling_window(double current_max_height, size_t max_number_of_elements,
    doublev* values, double* moving_average,
    double* std_dev)
{
    ...
    if (static_cast<double>(values->size()) > reset_window_size) {
        ...
        auto average_over_reset_window = std::accumulate(values->begin(),
                                             values->begin() + static_cast<int64_t>(reset_window_size),
                                             0.0)
            / reset_window_size;
        ...
        if (std::abs(average_over_reset_window - *moving_average) > (constants::deviation_amount_to_reset * (*std_dev))) {
            values->erase(values->begin(),
                values->begin() + static_cast<int64_t>((static_cast<double>(values->size()) * constants::auto_scaling_erase_percent)));
...
void spectrum_visualizer::create_spectrum_bars(fftw_complex* fftw_output, size_t fftw_results, int32_t win_height,
    ...
    doublev* bars, doublev* bars_falloff)
{
...
void spectrum_visualizer::recalculate_cutoff_frequencies(uint32_t number_of_bars,
    uint32v* low_cutoff_frequencies,
    ...
    doublev* freqconst_per_bin)
{
    ...
    for (auto i = 0u; i <= number_of_bars; i++) {
        ...
        (*low_cutoff_frequencies)[i] = static_cast<uint32_t>(std::floor(
            frequency * static_cast<double>(m_cfg->sample_size) / 4.0));
    ...
    for (auto i = 0u; i < number_of_bars; i++) {
        ...
        for (auto cutoff_freq = low_cutoff_frequencies[i];
            ...
            freq_magnitude += std::sqrt(
                (fftw_output[cutoff_freq][0] * fftw_output[cutoff_freq][0]) + (fftw_output[cutoff_freq][1] * fftw_output[cutoff_freq][1]));
        }
