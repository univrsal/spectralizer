...
namespace audio
{
    ...
    void spectrum_visualizer::scale_bars(int32_t height, doublev* bars)
    {
        ...
        const auto max_number_of_elements = static_cast<size_t>(
                ((AUTO_SCALE_SPAN * m_cfg->sample_rate) /
                 (static_cast<double>(m_cfg->sample_size))) *
                2.0);
    ...
    void spectrum_visualizer::recalculate_cutoff_frequencies(uint32_t number_of_bars,
        ...
        auto freq_const = std::log10((m_cfg->low_cutoff_freq /
                                     m_cfg->high_cutoff_freq)) /
                                     ((1.0 / number_of_bars + 1.0) - 1.0);
        ...
        for (auto i = 0u; i <= number_of_bars; i++) {
            (*freqconst_per_bin)[i] =
                    ...
                    std::pow(10.0, (freq_const * -1) +
                             (((i + 1.0) / (number_of_bars + 1.0)) * freq_const));

            auto frequency = (*freqconst_per_bin)[i] /
                    (m_cfg->sample_rate / 2.0);

                    (*low_cutoff_frequencies)[i] =
                    static_cast<uint32_t>(std::floor(
                                              frequency *
                                              static_cast<double>(m_cfg->sample_size) / 4.0));
